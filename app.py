...
cp = st.selectbox("Chỉ số đau ngực:", options=[0, 1, 2, 3])
trestbps = st.number_input("Huyết áp nghỉ (mm Hg):", min_value=0)
chol = st.number_input("Cholesterol (mg/dl):", min_value=0)
fbs = st.selectbox("Đường huyết lúc nhịn ăn:", options=[0, 1])
restecg = st.selectbox("Kết quả điện tâm đồ:", options=[0, 1, 2])
thalach = st.number_input("Tần số tim tối đa:", min_value=0)
exang = st.selectbox("Đau thắt ngực khi hoạt động:", options=[0, 1])
oldpeak = st.number_input("Độ dốc ST (0.0 - 6.2):", min_value=0.0, max_value=6.2)
slope = st.selectbox("Độ dốc của đỉnh ST:", options=[0, 1, 2])
ca = st.selectbox("Số mạch máu lớn:", options=[0, 1, 2, 3])
